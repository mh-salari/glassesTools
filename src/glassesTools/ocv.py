    ...
    def read_frame(self, report_gap=False, wanted_frame_idx: int = None) -> tuple[bool, np.ndarray, int, float]:
        ...
        while True:
            ...
            if self.frame_idx == 1 or ocv_ts > 0.0:
                ...
                if (
                ...
                ):  # little bit of leeway (1ms) for precision or mismatched timestamps
                    ...
                    tss = (
                        self._ts - self._last_good_ts[2]
                    )  # apply jump to our own timestamps (so, we're robust to e.g. OpenCV ignoring the edit list)
