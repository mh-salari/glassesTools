...
def from_plane_gaze(
    ...
    recFreq = np.round(np.mean(1000.0 / ts_diff))  # Hz
    knownFreqs = [30.0, 50.0, 60.0, 90.0, 120.0, 200.0]
    ...
    for idx, iv in enumerate(classification_intervals):
        ...
        if qHasRay:
            ...
            homography_x = np.array([
                s.gazePosPlane2D_vidPos_homography[0] for v in gazesToClassify.values() for s in v
            ])
            homography_y = np.array([
                s.gazePosPlane2D_vidPos_homography[1] for v in gazesToClassify.values() for s in v
            ])
