...
def getCameraFromTSLV(inputDir: str | pathlib.Path):
    ...
    with pathlib.Path(str(inputDir / "et.tslv")).open("rb") as f:
...
def json2df(jsonFile: str | pathlib.Path, sceneVideoDimensions: list[int]) -> pd.DataFrame:
    ...
    for entry in entries:
        ...
        # otherwise it contains gaze position data
            ...
            gazeData[entry["ts"]]["gaze_pos_vid_x"] = (
                entry["gp"][0] * sceneVideoDimensions[0] if not isError else math.nan
            )
            gazeData[entry["ts"]]["gaze_pos_vid_y"] = (
                entry["gp"][1] * sceneVideoDimensions[1] if not isError else math.nan
            )
