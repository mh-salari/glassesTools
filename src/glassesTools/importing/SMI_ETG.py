...
def checkRecording(inputDir: str | pathlib.Path, recInfo: str | pathlib.Path, use_return=False):
    ...
    if not (inputDir / file).is_file():
        if use_return:
            return False
        raise RuntimeError(
            f"{file} file not found in {inputDir}. Make sure you export the scene video using BeGaze as described in the glassesValidator manual."
        )
    ...
    if not (inputDir / file).is_file():
        if use_return:
            return False
        raise RuntimeError(
            f"{file} file not found in {inputDir}. Make sure you export the gaze data using BeGaze as described in the glassesValidator manual."
        )
...
def readSMICamInfoFile(inputDir: str | pathlib.Path):
    with pathlib.Path(inputDir / "codec1.bin").open("r") as file:
...
def getCameraFromFile(inputDir: str | pathlib.Path, outputDir: str | pathlib.Path):
    ...
    camera["resolution"] = np.array([
        camInfo.getint("MiiSimulation", "SceneCamWidth"),
        camInfo.getint("MiiSimulation", "SceneCamHeight"),
    ])
    camera["sensorOffsets"] = np.array([
        camInfo.getfloat("MiiSimulation", "SceneCamSensorOffsetX"),
        camInfo.getfloat("MiiSimulation", "SceneCamSensorOffsetY"),
    ])
    ...
    camera["eulerAngles"] = np.array([
        camInfo.getfloat("MiiSimulation", "SceneCamOrX"),
        camInfo.getfloat("MiiSimulation", "SceneCamOrY"),
        camInfo.getfloat("MiiSimulation", "SceneCamOrZ"),
    ])
...
def formatGazeData(inputDir: str | pathlib.Path, recInfo: Recording, sceneVideoDimensions: list[int]):
    ...
    df.insert(
        0, "frame_idx", (((df_fr.hour * 60 + df_fr.minute) * 60 + df_fr.second) * frameRate + df_fr.frame).to_numpy()
    )
...
def gazedata2df(textFile, sceneVideoDimensions):
    ...
    textData = pathlib.Path(textFile).read_text()
