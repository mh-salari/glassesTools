...
def importData(
    ...
    _store_data(
        output_dir, gazeData, frameTimestamps, rec_info, source_dir_as_relative_path=source_dir_as_relative_path
    )
...
def getRecordingInfo(inputDir: str | pathlib.Path) -> Recording:
    ...
    for f in ("metadata.json", "worldCamera.mp4", "gaze.tsv", "calibration.xml"):
        if not (inputDir / f).is_file():
            print(
                f"This directory does not contain a valid {EyeTracker.Meta_Aria_Gen_1.value} recording export. The {f} file is not found in the input directory {inputDir}. Make sure you run the meta_aria_gen1_exporter.py script on the recording's vrs file"
            )
            return None

    with pathlib.Path(inputDir / "metadata.json").open() as f:
...
def checkRecording(inputDir: str | pathlib.Path, recInfo: Recording):
    ...
    if recInfo.eye_tracker != actualRecInfo.eye_tracker:
        raise ValueError(
            f'A recording for a "{recInfo.eye_tracker.value}" device was not found in the folder {inputDir}.'
        )
...
def copyRecording(inputDir: pathlib.Path, outputDir: pathlib.Path, rec_info: Recording, copy_scene_video: bool):
    ...
    frameIdx = video_utils.timestamps_to_frame_number(
        gazeData.loc[:, "timestamp"].values, frameTimestamps["timestamp"].values
    )
