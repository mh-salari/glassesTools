...
json.register_type(
    json.TypeEntry(
        Type, "__enum.gaze_worldref.Type__", utils.enum_val_2_str, lambda x: getattr(Type, x.split(".")[-1])
    )
)
...
class Gaze:
    ...
    _columns_compressed: ClassVar[dict[str, int]] = {
        ...
        "frame_idx_VOR": 1,
        "frame_idx_ref": 1,
        "gazePosCam_vidPos_ray": 3,
        "gazePosCam_vidPos_homography": 3,
        "gazePosCamWorld": 3,
        "gazeOriCamLeft": 3,
        "gazePosCamLeft": 3,
        "gazeOriCamRight": 3,
        "gazePosCamRight": 3,
        "gazePosPlane2D_vidPos_ray": 2,
        "gazePosPlane2D_vidPos_homography": 2,
        "gazePosPlane2DWorld": 2,
    ...
    def draw_on_world_video(
        ...
        if not camera_params.has_intrinsics():
            if pose is not None and pose.homography_successful():
                ...
                _draw(
                    img,
                    pose.plane_to_cam_homography(self.gazePosPlane2D_vidPos_homography, camera_params),
                    3,
                    clr_vidPos,
                )
...
def _from_head_impl(pose: pose.Pose, gaze: gaze_headref.Gaze, camera_params: ocv.CameraParams) -> Gaze:
    ...
    if pose.pose_successful():
        ...
        gaze_world.gazePosPlane2D_vidPos_ray, gaze_world.gazePosCam_vidPos_ray = pose.cam_to_plane_pose(
            gaze.gaze_pos_vid, camera_params
        )
    ...
    if pose.homography_successful():
        ...
        if pose.pose_successful():
            gaze_world.gazePosCam_vidPos_homography = pose.world_frame_to_cam(
                np.append(gaze_world.gazePosPlane2D_vidPos_homography, 0)
            )
...
def distance_from_plane(gaze: Gaze, plane: plane.Plane) -> float:
    ...
    if gaze.gazePosPlane2D_vidPos_ray is not None and not math.isnan(gaze.gazePosPlane2D_vidPos_ray[0]):
        ...
        gaze.gazePosPlane2D_vidPos_homography[0]
    ):
