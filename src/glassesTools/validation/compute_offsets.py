...
def compute(
    ...
    if not isinstance(marker_intervals, pd.DataFrame):
        marker_intervals = pd.read_csv(
            marker_intervals, delimiter="\t", dtype={"marker_interval": int}, index_col=["marker_interval", "target"]
        )
    if not all(
        c in marker_intervals or c in marker_intervals.index.names
        for c in ("marker_interval", "target", "start_timestamp", "end_timestamp")
    ):
        raise ValueError(
            "Provided marker intervals should contain the following columns: [marker_interval, target, start_timestamp, end_timestamp]"
        )
    ...
    for idx, iv in enumerate(validation_intervals):
        ...
        for dt in d_types:
            df_idx.append(
                np.vstack((np.full((idxs.shape[0],), idx + 1, dtype="int"), idxs.shape[0] * [dt], idxs[:, 0])).T
            )
        df_idx = pd.DataFrame(
            np.vstack(tuple(df_idx)),
            columns=[marker_intervals.index.names[0], "type", marker_intervals.index.names[1]],
        )
        df = pd.DataFrame(
            index=pd.MultiIndex.from_frame(
                df_idx.astype({
                    marker_intervals.index.names[0]: "int64",
                    "type": "category",
                    marker_intervals.index.names[1]: "int64",
                })
            )
        )
        ...
        for dt in d_types:
            df.loc[idxer[idx + 1, dt, :], "order"] = (
                np.argsort(marker_intervals.loc(axis=0)[idx + 1, :]["start_timestamp"].to_numpy()) + 1
            )
        ...
        for t in targets:
            ...
            with warnings.catch_warnings():
                ...
                df.loc[idxer[idx + 1, :, t], "acc"] = np.hypot(
                    df.loc[idxer[idx + 1, :, t], "acc_x"], df.loc[idxer[idx + 1, :, t], "acc_y"]
                )
                ...
                df.loc[idxer[idx + 1, :, t], "rms"] = np.hypot(
                    df.loc[idxer[idx + 1, :, t], "rms_x"], df.loc[idxer[idx + 1, :, t], "rms_y"]
                )

                df.loc[idxer[idx + 1, :, t], "std_x"] = np.nanstd(offset[:, :, 0], ddof=0, axis=0)
                df.loc[idxer[idx + 1, :, t], "std_y"] = np.nanstd(offset[:, :, 1], ddof=0, axis=0)
                df.loc[idxer[idx + 1, :, t], "std"] = np.hypot(
                    df.loc[idxer[idx + 1, :, t], "std_x"], df.loc[idxer[idx + 1, :, t], "std_y"]
                )

                if include_data_loss:
                    df.loc[idxer[idx + 1, :, t], "data_loss"] = (
