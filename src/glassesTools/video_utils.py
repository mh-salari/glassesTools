...
def _get_isobmmf_timestamps(vid_file: pathlib.Path) -> np.ndarray | None:
    ...
    if edts_idx:
        ...
        for i in range(elst.box_info["entry_count"]):
            if i == edit_start_index and elst.box_info["entry_list"][i]["media_time"] == -1:
                ...
                this_empty_duration = np.int64(
                    elst.box_info["entry_list"][i]["segment_duration"]
                )  # NB: in movie time scale
                ...
                media_start_time = np.int64(
                    elst.box_info["entry_list"][i]["media_time"]
                )  # NB: already in track timescale, do not scale
                media_duration = av_rescale(
                    np.int64(elst.box_info["entry_list"][i]["segment_duration"]), media_time_scale, movie_time_scale
                )  # as above, scale to track timescale
                if media_start_time < 0:
                    raise RuntimeError(
                        "File contains an edit list that is too complicated (media start time < 0) for this parser, not supported"
                    )
                if elst.box_info["entry_list"][i]["media_rate"] != 1.0:
                    raise RuntimeError(
                        "File contains an edit list that is too complicated (media time is not 1.0) for this parser, not supported"
                    )
    ...
    dts = np.delete(
        dts, -1
    )  # remove last, since that denotes _end_ of last frame, and we only need timestamps for frame onsets
    # apply ctts
    if ctts_idx:
        dts += ctts
...
def _get_frame_timestamps_opencv(vid_file: pathlib.Path) -> np.array:
    ...
    while vid.isOpened():
        ...
        if not ret and frame_idx > 0 and frame_idx / nframes < 0.99:
            vid.release()
            raise RuntimeError(
                "The video file is corrupt. Testing has shown that it cannot be guaranteed that timestamps remain correct when trying to read past the hole. So abort, cannot process this video."
            )
...
def _get_frame_timestamps_from_video(vid_file: pathlib.Path) -> np.array:
    if is_isobmmf(vid_file):
        ...
        if ts is None:
            ts = _get_frame_timestamps_ffprobe(vid_file)
        return ts
    ts = _get_frame_timestamps_ffprobe(vid_file)
    if ts is None:
        ts = _get_frame_timestamps_opencv(vid_file)
    return ts
