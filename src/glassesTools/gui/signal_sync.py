...
import glfw
import numpy as np
from imgui_bundle import glfw_utils, hello_imgui, imgui, immapp, implot
...
class GUI:
    def __init__(self, use_thread=True):
        ...
        self.offset_t: float = 0.0
        self.offset_t_ori: float = 0.0
        self._offset_xy: list[float] = [0.0, 0.0]
    ...
    def set_data(
        ...
        title: str,
        ival: int,
        gazes: dict[int, list[gaze_headref.Gaze]],
        target_positions: dict[int, TargetPos],
        offset_t=0.0,
    ):
        if not self.is_running():
            raise RuntimeError(
                "You can only call this function once the GUI is actually running. check GUI.is_running()"
            )
        ...
        with self._data_lock:
            ...
            self.gaze_data["ts"] = (
                np.array([s.timestamp for fr in gazes for s in gazes[fr]], "float") / 1000
            )  # ms -> s
            ...
            self.target_data["ts"] = (
                np.array([target_positions[fr].timestamp for fr in target_positions], "float") / 1000
            )  # ms -> s
            ...
            if implot.begin_subplots("##xy_plots", 2, 1, (-1, -1), implot.SubplotFlags_.link_all_x):
                for d in range(2):
                    ...
                    if implot.begin_plot("##X", flags=implot.Flags_.no_mouse_text):
                        ...
                        implot.setup_axis(
                            implot.ImAxis_.y1, "horizontal coordinate (pix)" if d == 0 else "vertical coordinate (pix)"
                        )
                        if self._hovered == d:
                        ...
                        if d == 0:
                            ...
                            implot.annotation(
                                ax_lims.x.max,
                                ax_lims.y.min,
                                implot.get_style().color_(implot.Col_.inlay_text),
                                (0, 0),
                                True,
                                f"offset: {(self.offset_t + self._temp_off[0]) * 1000:.1f}ms",
                            )
                        implot.end_plot()
    ...
    def _do_data_drag(self, ax_idx: int):
        ...
        if (
            self._last_mouse_pos[ax_idx] is None
            or self._last_mouse_pos[ax_idx].x != p.x
            or (invalidate1 := (self._plot_size[ax_idx] is None or self._plot_size[ax_idx] != plot_size))
            or (invalidate2 := (self._plot_limits[ax_idx] is None or self._plot_limits[ax_idx] != plot_limits))
        ):
            ...
            if distance < 4.0 * hello_imgui.dpi_window_size_factor():
                # hovering
    ...
    def _distance_to_gaze_signal(self, pos: implot.Point, ax: str):
        if self._gaze_data_plot_pix[ax] is None:
            self._gaze_data_plot_pix[ax] = np.array([
                (p.x, p.y)
                for p in (
                    implot.plot_to_pixels(implot.Point(x, y))
                    for x, y in zip(self._gaze_data_offset["ts"], self._gaze_data_offset[ax])
                )
            ])

        pos = implot.plot_to_pixels(pos)
        return np.nanmin(
            np.hypot(self._gaze_data_plot_pix[ax][:, 0] - pos.x, self._gaze_data_plot_pix[ax][:, 1] - pos.y)
        )
