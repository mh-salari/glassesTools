...
def color_brighten(color: imgui.ImColor, amount: float):
    amount = 1.0 / (1.0 + amount)
    return imgui.ImColor(
        1.0 - (1.0 - color.value.x) * amount,
        1.0 - (1.0 - color.value.y) * amount,
        1.0 - (1.0 - color.value.z) * amount,
        color.value.w,
    )
...
class Timeline:
    def __init__(
        ...
        self._duration = self._video_ts.get_last()[1] / 1000.0  # ms -> s
        if self._duration == 0.0:
            raise ValueError(
                "Cannot make a timeline for a video with 0s duration, there may be something wrong with the frame timestamps that were passed in"
            )
        ...
        self._annotation_tooltips: dict[str, str] = {}
    ...
    def set_allow_annotate(self, allow_annotate: set[str]):
    ...
    def set_annotation_keys(
        self, annotate_shortcut_key_map: dict[str, imgui.Key], annotate_tooltips: dict[str, str] | None = None
    ):
        ...
        for e in self._annotations_frame:
            ...
            if annotate_tooltips and e in annotate_tooltips:
                ...
                if e in self._allow_annotate:
                    ...
                    tool_tip += " (read only)"
            elif e in self._allow_annotate:
                if e in annotate_shortcut_key_map:
    ...
    def _annotations_to_time(self) -> dict[str, list[float]] | None:
        if not self._annotations_frame:
            return None
        return {
            e: [self._video_ts.get_timestamp(i) / 1000 for i in self._annotations_frame[e]]
            for e in self._annotations_frame
        }  # ms -> s

    def _make_annotation_colors(self) -> dict[str, imgui.ImColor]:
        if not self._annotations_frame:
            return {}
    ...
    def get_annotation_colors(self) -> dict[str, imgui.ImColor]:
    ...
    def _request_delete(self, event: str, frame_idxs: list[int]):
    ...
    def _determine_ticks(self):
        ...
        major_ticks_time = np.array([
            t for t in self._major_ticker.tick_values(0, self._duration) if t >= 0 and t <= self._duration
        ])
    ...
    def _draw_time_ruler(self):
        ...
        if self._allow_seek:
            ...
            if imgui.is_item_activated() or (imgui.is_item_active() and abs(drag_delta.x) > 0.001):
        ...
        for lbl, pos in zip(self._major_ticks_lbl, self._major_ticks_pos):
            ...
            draw_list.add_text(
                (rounded_gridline_pos_x + 4 * dpi_fac, cursor_pos.y + 2 * style.frame_padding.y - 2 * dpi_fac),
                lbl_color,
                lbl,
            )
            draw_list.add_line(
                (rounded_gridline_pos_x, tick_pos_y - 10 * dpi_fac),
                (rounded_gridline_pos_x, tick_pos_y - 2 * dpi_fac),
                tick_color,
                thickness=dpi_fac,
            )

        # draw play head
        playhead_screen_position = self._time_to_pos(
            self._dragged_time if self._dragged_time is not None else self._current_time, True
        )
        if self.get_num_annotations():
            ...
            draw_list.add_line(
                playhead_line_pos, playhead_line_pos + imgui.ImVec2(0, size.y), 0xE553A3F9, thickness=2 * dpi_fac
            )
    ...
    def _handle_zoom(self, is_hovered: bool):
        ...
        if is_hovered and mouse_wheel != 0.0:
            # clamp scaling, scale should not get smaller than 1
            delta_scale = 0.25 * mouse_wheel
            delta_scale = max(1.0, self._scale + delta_scale) - self._scale
            if np.isclose(0.0, delta_scale):
                return

            self._scale += delta_scale
            new_h_scroll = min(max(0.0, self._h_scroll + mouse_pos.x * delta_scale), self.draw_width)
            if not np.isclose(self._h_scroll, new_h_scroll):
                self._new_h_scroll = new_h_scroll

    def _timepoint_interaction_logic(
        self,
        lbl: str,
        x_pos: float,
        y_ext: float,
        event: str | None,
        hover_info: tuple[str, float, int] | None = None,
        draggable=False,
        has_context_menu=True,
        add_episode_action=False,
        ...
        if self._allow_seek or (event in self._allow_annotate) or has_context_menu or hover_info:
            ...
            if hover_info and imgui.is_item_hovered(
                imgui.HoveredFlags_.for_tooltip | imgui.HoveredFlags_.delay_normal
            ):
                imgui.set_tooltip(
                    f"{hover_info[0]} {hover_info[1]}\ntimestamp: {hover_info[2]:.3f}\nframe index: {hover_info[3]}"
                )
            ...
            if (
                has_context_menu
                and (self._allow_seek or event in self._allow_annotate)
                and imgui.begin_popup_context_item(f"##timepoint_context_menu_{lbl}")
            ):
                if self._allow_seek and imgui.selectable(ifa6.ICON_FA_ARROW_RIGHT + " go to timepoint", False)[0]:
                    do_move = True
                if (
                    event in self._allow_annotate
                    and imgui.selectable(ifa6.ICON_FA_TRASH_CAN + " remove timepoint", False)[0]
                ):
                    action = "delete_timepoint"
                if (
                    event in self._allow_annotate
                    and add_episode_action
                    and imgui.selectable(ifa6.ICON_FA_TRASH_CAN + " remove episode", False)[0]
                ):
    ...
    def _episode_interaction_logic(
        self,
        lbl: str,
        x_start: float,
        x_end: float,
        y_ext: float,
        event: str | None,
        hover_info: tuple[str, int, list[float], list[int]] = None,
    ) -> tuple[float, bool, str | None]:
        x_pos = x_start
        do_move = False
        action = None
        ...
        if self._allow_seek or (event in self._allow_annotate) or hover_info:
            ...
            if (self._allow_seek or event in self._allow_annotate) and imgui.begin_popup_context_item(
                f"##episode_context_menu_{lbl}"
            ):
    ...
    def _draw_tracks(self, parent_cursor_pos, parent_size, height):
        ...
        for i in range(1, len(self._major_ticks_pos), 2):
            ...
            draw_list.add_rect_filled(
                (cursor_pos.x + start, cursor_pos.y), (cursor_pos.x + end, cursor_pos.y + size.y), section_color
            )
        ...
        for e in self._annotations_frame:
            self._draw_track(
                e,
                self._annotation_types[e],
                self._annotations[e],
                self._annotations_frame[e],
                height_per_track,
                self._annotation_colors[e],
                text_size,
            )
        imgui.pop_style_var()
        ...
        new_pos, should_process, _ = self._timepoint_interaction_logic(
            "player_bar", playhead_screen_position, size.y, None, draggable=True, has_context_menu=False
        )
        if should_process:
    ...
    def _draw_track(
        ...
        annotations_time: list[float],
        annotations_frame: list[int],
        height: float,
        color: imgui.ImColor,
        ...
        draw_list.add_line((cursor_pos.x, y_pos), (cursor_pos.x + size.x, y_pos), grid_color, 2 * dpi_fac)

        # setup for other drawing
        text_contrast_ratio = 1 / 1.57
        text_color = imgui.ImColor(imgui.get_style_color_vec4(imgui.Col_.text))
        text_color_adjusted = imgui.color_convert_float4_to_u32(
            color_adjust_contrast(text_color, text_contrast_ratio, color).value
        )
        ...
        if annotation.type_map[event_type] == annotation.Type.Interval:
            ...
            for m in range(0, len(annotations_time) - 1, 2):  # -1 to make sure we don't try incomplete intervals
                ...
                draw_list.add_rect_filled(
                    (start_screen_position, cursor_pos.y), (end_screen_position, cursor_pos.y + size.y), bg_color
                )
                draw_list.add_rect(
                    (start_screen_position, cursor_pos.y + dpi_fac / 2),
                ...
                hover_info = (
                    (event, int(m / 2) + 1, annotations_time[m], annotations_frame[m])
                    if self._show_info_on_hover
                    else None
                )
                _, do_move, action = self._timepoint_interaction_logic(
                    f"annotation_{event}_{m}",
                ...
                hover_info = (
                    (event, int(m / 2) + 1, annotations_time[m + 1], annotations_frame[m + 1])
                    if self._show_info_on_hover
                    else None
                )
                _, do_move, action = self._timepoint_interaction_logic(
                    f"annotation_{event}_{m + 1}",
                    end_screen_position,
                    size.y,
                    event,
                    hover_info,
                ...
                hover_info = (
                    (event, int(m / 2) + 1, annotations_time[m : m + 2], annotations_frame[m : m + 2])
                    if self._show_info_on_hover
                    else None
                )
                x_pos, do_move, action = self._episode_interaction_logic(
                    f"annotation_{event}_{m}", start_screen_position, end_screen_position, size.y, event, hover_info
                )
                if do_move:
            ...
            if len(annotations_time) % 2:
                ...
                action = self._draw_annotation_line(
                    f"{event} unmatched!",
                    event,
                    len(annotations_time) - 1,
                    annotations_time[-1],
                    annotations_frame[-1],
                    math.ceil(len(annotations_time) / 2),
                    size.y,
                    border_color,
                )
            ...
            for i, (at, af) in enumerate(zip(annotations_time, annotations_frame)):
                action = self._draw_annotation_line(event, event, i, at, af, i + 1, size.y, border_color)
                if action == "delete_timepoint":
        ...
        if self._show_annotation_labels:
            ...
            if self._annotation_tooltips.get(event):
    ...
    def draw(self):
        ...
        imgui.begin_child(
            "##ruler",
            size=(self.draw_width, imgui.get_font_size() + 2 * imgui.get_style().frame_padding.y),
            child_flags=imgui.ChildFlags_.borders,
            window_flags=imgui.WindowFlags_.no_background
            | imgui.WindowFlags_.no_scrollbar
            | imgui.WindowFlags_.no_scroll_with_mouse,
        ...
        if self._annotations_frame:
            ...
            imgui.begin_child(
                ...
                size=(self.draw_width, 0),
                child_flags=imgui.ChildFlags_.borders,
                window_flags=imgui.WindowFlags_.no_background | imgui.WindowFlags_.no_scroll_with_mouse,
            )
