...
if platform.os == platform.Os.Windows:
    ...
    def _error_check_non0_is_error_ex(allowed, result, func, args):
        ...
        if result == ERROR_EXTENDED_ERROR:
            ...
            WNetGetLastError(
                ctypes.byref(last_error), description, ctypes.sizeof(description), provider, ctypes.sizeof(provider)
            )
    ...
    WNetAddConnection2.errcheck = lambda r, f, a: _error_check_non0_is_error_ex(
        [ERROR_SESSION_CREDENTIAL_CONFLICT], r, f, a
    )
    ...
    NetShareEnum.argtypes = (
        LMSTR,
        ctypes.wintypes.DWORD,
        ctypes.POINTER(ctypes.wintypes.LPBYTE),
        ctypes.wintypes.DWORD,
        ctypes.wintypes.LPDWORD,
        ctypes.wintypes.LPDWORD,
        ctypes.wintypes.LPDWORD,
    ...
    def get_thispc_listing() -> list[DirEntry]:
        ...
        for d, subdir, disp_name in [
            ...
            (CSIDL_PROFILE, "Downloads", "Downloads"),
        ]:
            ...
            if bitmask & 1:
                ...
                match GetDriveType(drive):
                    ...
                    case 2:  # DRIVE_REMOVABLE
                        ...
                        display_name = drive_name.value or "USB Drive"
                        entry.name = f"{display_name} ({drive[0:-1]})"
                    case 3:  # DRIVE_FIXED
                        entry.mime_type = "file_action/drive"
                        display_name = drive_name.value or "Local Disk"
                        entry.name = f"{display_name} ({drive[0:-1]})"
                    case 4:  # DRIVE_REMOTE
        ...
        while True:
            ...
            for idx in range(cEntries.value):
                ...
                if len(path_comps) >= 2:
                    shares.append(
                        DirEntry(
                            path_comps[1], True, pathlib.Path(share_path), None, None, None, "file_action/net_share"
                        )
                    )
        ...
        for share in remote_shares:
            shares.append(
                DirEntry(
                    share.netname,
                    True,
                    pathlib.Path(f"\\\\{server}\\{share.netname}"),
                    None,
                    None,
                    None,
                    "file_action/net_share",
                )
            )
